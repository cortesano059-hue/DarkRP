...
const userSchema = new mongoose.Schema(
  {
    ...
    // 🔥 Campos para cooldowns que usa economy.js
...
const itemSchema = new mongoose.Schema({
  ...
  emoji: { type: String, default: "📦" }, // emoji asignado

  data: { type: Object, default: {} }, // extras opcionales
});
...
const backpackSchema = new mongoose.Schema(
  {
    ...
    name: { type: String, required: true }, // nombre de la mochila
    emoji: { type: String, default: "🎒" },
    description: { type: String, default: "" },

    // Capacidad = número de tipos de items, NO cantidades
    capacity: { type: Number, default: 15 },

    // owner_only → dueño + admins
    // custom     → dueño + allowedUsers + allowedRoles + admins
    accessType: {
      type: String,
...
// Unicidad por dueño + servidor + nombre
...
workSchema.index({ userId: 1, guildId: 1 }, { unique: true });

const Work = getModel("Work", workSchema);
...
dailySchema.index({ userId: 1, guildId: 1 }, { unique: true });

const Daily = getModel("Daily", dailySchema);
