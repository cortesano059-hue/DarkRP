...
const eco = require("@economy"); // tu API de economía / items
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!isAdmin) return safeReply(interaction, "⛔ Necesitas permisos de administrar el servidor.", true);
    ...
    const draft = {
      ...
      requirements: [],
      actions: [],
      _editing: null, // if editing existing item doc
    };

    // If edit requested and item exists, load
    ...
    // Buttons: Requisitos, Acciones, Propiedades, Vista previa, Guardar, Cancelar
    ...
    const reply = await interaction.followUp({ embeds: [makeEmbed()], components: [rowMain, rowMain2], ephemeral: true });
    ...
    collector.on("collect", async btnInt => {
      try {
        ...
        if (btnInt.customId === "reqs") {
          // send a select menu to pick req type
          ...
          const msg = await interaction.followUp({ content: "Selecciona el tipo de requisito a añadir:", components: [selRow], ephemeral: true });
          const sColl = msg.createMessageComponentCollector({ componentType: ComponentType.StringSelect, time: 60_000, max: 1, filter: i => i.user.id === interaction.user.id });

          sColl.on("collect", async s => {
            await s.deferUpdate();
            const v = s.values[0];

            // ROLE: use RoleSelectMenu
            if (v === "role") {
              ...
              const rmsg = await interaction.followUp({ content: "Selecciona el rol requerido:", components: [roleRow], ephemeral: true });
              const rColl = rmsg.createMessageComponentCollector({ componentType: ComponentType.RoleSelect, time: 60_000, max: 1, filter: i => i.user.id === interaction.user.id });

              rColl.on("collect", async rr => {
                await rr.deferUpdate();
                const selectedRole = rr.roles[0];
                // Ask if mustHave or mustNotHave via buttons
                const mustRow = new ActionRowBuilder().addComponents(
                ...
                const mmsg = await interaction.followUp({ content: `Rol seleccionado: <@&${selectedRole.id}> — ¿Debe tenerlo o no?`, components: [mustRow], ephemeral: true });
                const mColl = mmsg.createMessageComponentCollector({ componentType: ComponentType.Button, time: 60_000, max: 1, filter: i => i.user.id === interaction.user.id });

                mColl.on("collect", async mm => {
              ...
              // Use Modal to enter amount and operator
              ...
              const modalSub = await interaction.awaitModalSubmit({ time: 60_000, filter: (m) => m.user.id === interaction.user.id }).catch(()=>null);
              if (!modalSub) {
              ...
              // Modal asking item name and amount
          ...
          }); // end select collector

          sColl.on("end", () => {
            // remove select message optionally
          });
        ...
        // ACCIONES FLOW
          ...
          const msg = await interaction.followUp({ content: "Selecciona el tipo de acción:", components: [selRow], ephemeral: true });
          const sColl = msg.createMessageComponentCollector({ componentType: ComponentType.StringSelect, time: 60_000, max: 1, filter: i => i.user.id === interaction.user.id });
          ...
          sColl.on("collect", async s => {
            ...
            if (v === "message") {
              ...
              // Ask add roles then remove roles via RoleSelectMenu
              ...
              const remRow = new ActionRowBuilder().addComponents(
                new RoleSelectMenuBuilder().setCustomId("roles_rem").setPlaceholder("Selecciona roles a QUITAR").setMinValues(0).setMaxValues(5)
              );
              const addMsg = await interaction.followUp({ content: "Selecciona roles a AÑADIR (puedes saltar):", components: [addRow], ephemeral: true });
              const addColl = addMsg.createMessageComponentCollector({ componentType: ComponentType.RoleSelect, time: 60_000, max: 1, filter: i => i.user.id === interaction.user.id });

              addColl.on("collect", async rr => {
                await rr.deferUpdate();
                const adds = rr.roles.map(r => r.id);
                const remMsg = await interaction.followUp({ content: "Selecciona roles a QUITAR (puedes saltar):", components: [remRow], ephemeral: true });
                const remColl = remMsg.createMessageComponentCollector({ componentType: ComponentType.RoleSelect, time: 60_000, max: 1, filter: i => i.user.id === interaction.user.id });

                remColl.on("collect", async rrr => {
                  await rrr.deferUpdate();
                  const rems = rrr.roles.map(r => r.id);
                  draft.actions.push({
                    type: "roles",
                    addRoles: adds,
                    removeRoles: rems
                ...
                remColl.on("end", async (collected) => {
                  if (collected.size === 0) {
                    // user skipped remove roles
                    draft.actions.push({
                      type: "roles",
                      addRoles: adds,
                      removeRoles: []
              ...
              addColl.on("end", async (collected) => {
                if (collected.size === 0) {
                  // user skipped add roles, still ask remove
                  const remMsg = await interaction.followUp({ content: "Selecciona roles a QUITAR (puedes saltar):", components: [remRow], ephemeral: true });
                  const remColl = remMsg.createMessageComponentCollector({ componentType: ComponentType.RoleSelect, time: 60_000, max: 1, filter: i => i.user.id === interaction.user.id });
        ...
        // PROPERTIES flow
          ...
          const propsRow = new ActionRowBuilder().addComponents(
          ...
          const propsRow2 = new ActionRowBuilder().addComponents(
          ...
          const pMsg = await interaction.followUp({ content: "Elige propiedad a editar:", components: [propsRow, propsRow2], ephemeral: true });

          const pColl = pMsg.createMessageComponentCollector({ componentType: ComponentType.Button, time: 60_000, max: 1, filter: i => i.user.id === interaction.user.id });
          pColl.on("collect", async pc => {
            await pc.deferUpdate();
            if (pc.customId === "p_name") {
              const modal = new ModalBuilder().setCustomId("modal_name").setTitle("Editar nombre/desc");
              modal.addComponents(
                new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("nname").setLabel("Nombre").setStyle(TextInputStyle.Short).setRequired(true).setValue(draft.itemName)),
                new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("ndesc").setLabel("Descripción").setStyle(TextInputStyle.Paragraph).setRequired(false).setValue(draft.description || ""))
              );
              await interaction.showModal(modal);
              const mod = await interaction.awaitModalSubmit({ time: 60_000, filter: m => m.user.id === interaction.user.id }).catch(()=>null);
              if (mod) {
              ...
              modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("price").setLabel("Precio (número)").setStyle(TextInputStyle.Short).setRequired(true).setValue(String(draft.price || 0))));
              await interaction.showModal(modal);
              const mod = await interaction.awaitModalSubmit({ time: 60_000, filter: m => m.user.id === interaction.user.id }).catch(()=>null);
              if (mod) {
              ...
              const fmsg = await interaction.followUp({ content: "Toggle flags:", components: [flagsRow], ephemeral: true });
              const fColl = fmsg.createMessageComponentCollector({ componentType: ComponentType.Button, time: 60_000, max: 3, filter: i => i.user.id === interaction.user.id });

              fColl.on("collect", async f => {
                await f.deferUpdate();
        ...
        // SAVE
          ...
          try {
            // Build payload for eco.createItem
            const itemData = {
            ...
            // If editing existing doc, modify and save
            if (draft._editing) {
              ...
              // create (eco.createItem signature in your project accepts a data param)
              const created = await eco.createItem(guildId, draft.itemName, draft.description, draft.price, draft.emoji, itemData);
              if (!created) {
